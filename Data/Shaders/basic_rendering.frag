...
float shadow_pcf3x3(vec3 light_space_xyz){
	float shadow = 0.0;
	vec2 texelSize =1.0 / textureSize(shadowmap_texture,0);
	float z = light_space_xyz.z - uniform_constant_bias;
	
	for(int x=-1;x<=1;x++){
		for(int y=-1;y<=1;y++){
			float pcfDept = texture(shadowmap_texture,light_space_xyz.xy + vec2(x,y)*texelSize).r;
			shadow += z < pcfDept ? 1.0 : 0.0;
		}
	}
